...
{
    return std::count_if(
        blockIds.begin(), blockIds.end(), [](SizeType32 id) { return id != KVCacheBlock::kInvalidBlockId; });
}
} // namespace
...
TEST_F(KVCacheManagerTest, KVCacheManagerMaxAttentionWindowSmallerThanBlockSizeTest)
{
    ...
    EXPECT_THAT(seq0.getCacheBlockIds(onlyWindowSize).at(beamIdx),
        ::testing::ElementsAreArray({KVCacheBlock::kInvalidBlockId, 1}));
    ...
    EXPECT_EQ(blockManager.getNumFreeBlocks(), blocksInPrimaryPool - numValidBlocks);
    EXPECT_THAT(seq0.getCacheBlockIds(onlyWindowSize).at(beamIdx),
        ::testing::ElementsAreArray({KVCacheBlock::kInvalidBlockId, 1, 2}));
