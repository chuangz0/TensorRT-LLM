...
"""CPU-only tests for the hop-reduction experiments (pregrant / C++ chain).
...
class Ids:
    """One id sequence shared by pool/agent fakes (mirrors the real
    CompletionPoller's single id namespace)."""
...
class FakePeer:
    """Raw pyzmq sockets impersonating a remote reactor (one ROUTER to
    receive the reactor's sends, one DEALER to inject control messages)."""
...
# --------------------------------------------------------------------------- #
# Opt A: pregrant (TRTLLM_BOUNCE_V2_EXP_PREGRANT)
# --------------------------------------------------------------------------- #


def _send_want(h: Harness, rid: int, n_chunks: int) -> None:
    h.peer.send(codec.encode_want(rid, [K_PAGE] * n_chunks, h.peer.endpoint))


def test_pregrant_receiver_grants_whole_request(harness_factory) -> None:
    """env on + peer advertised CAP_PREGRANT -> ONE GRANT with all chunks."""
    h = harness_factory(make_config(enable_pregrant=True))
    h.reactor.set_peer_caps("peerA", codec.CAP_PREGRANT)
    _send_want(h, rid=7, n_chunks=12)
...
def test_pregrant_disabled_env_keeps_window(harness_factory) -> None:
    """env off -> the default in-flight window even if the peer advertised."""
    h = harness_factory(make_config(enable_pregrant=False))
    h.reactor.set_peer_caps("peerA", codec.CAP_PREGRANT)
    _send_want(h, rid=8, n_chunks=12)
    header, blob = h.peer.recv_typed(codec.BounceMsgType.GRANT)
    credits = codec.decode_credits(blob, header)
    assert credits is not None and len(credits) == 4  # max_inflight
    assert h.sched.held_count(f"peerA\x1f8") == 4


def test_pregrant_peer_without_cap_falls_back(harness_factory) -> None:
    """env on but the peer did NOT advertise CAP_PREGRANT (mixed-switch
    deployment) -> byte-for-byte the baseline windowed behavior."""
    h = harness_factory(make_config(enable_pregrant=True))
    # no set_peer_caps: a WANT-bootstrapped or capability-less peer
    _send_want(h, rid=9, n_chunks=12)
    header, blob = h.peer.recv_typed(codec.BounceMsgType.GRANT)
    credits = codec.decode_credits(blob, header)
...
def test_pregrant_refills_stop_after_full_grant(harness_factory) -> None:
    """After a full pregrant, scatter completions must NOT produce refill
    GRANTs (nothing pending) — the per-chunk grant hop is gone."""
    h = harness_factory(make_config(enable_pregrant=True))
    h.reactor.set_peer_caps("peerA", codec.CAP_PREGRANT)
    _send_want(h, rid=10, n_chunks=6)
    header, blob = h.peer.recv_typed(codec.BounceMsgType.GRANT)
    credits = codec.decode_credits(blob, header)
    assert credits is not None and len(credits) == 6
    # Complete every chunk: send DATA (empty scatter runs = vacuous success).
    empty_runs = np.empty(0, dtype=codec.SCATTER_RUN_DTYPE)
    for idx, credit in enumerate(credits):
        h.peer.send(codec.encode_data(10, idx, 6, credit.region_handle, empty_runs))
    # All chunks ACK back (batched), and no further GRANT ever arrives.
    got = 0
    while got < 6:
        header, blob = h.peer.recv_typed(codec.BounceMsgType.ACK)
        entries = codec.decode_ack(blob, header)
        assert entries is not None
        got += len(entries)
    assert h.sched.held_count(f"peerA\x1f10") == 0
...
def test_handshake_decode_with_and_without_caps() -> None:
    engine_mod = importlib.import_module("bounce_v2.engine")
    ep = b"tcp://127.0.0.1:1234"
    head = engine_mod._HANDSHAKE_HEADER.pack(
    ...
    decode = engine_mod.BounceEngine._decode_handshake
    # pre-capability blob (old build): caps default to 0
    parsed = decode(head + ep)
    assert parsed is not None
    assert parsed[4] == ep.decode() and parsed[5] == 0
    # capability blob: trailing u32 parsed
    parsed = decode(head + ep + engine_mod._HANDSHAKE_CAPS.pack(codec.CAP_PREGRANT))
    assert parsed is not None
    assert parsed[5] == codec.CAP_PREGRANT
